...
addon.Menu.EMOTES_PER_PAGE = 9 -- Option 1 is "Add Emote", 9 slots available for emotes per page
...
function addon.Menu.GetTotalPages(totalEmotes)
    if not totalEmotes or totalEmotes <= 0 then
        return 1
    end
    return math.max(1, math.ceil(totalEmotes / addon.Menu.EMOTES_PER_PAGE))
end
...
function addon.Menu.GetPageSlice(emotes, pageIndex)
    emotes = emotes or {}
    local totalPages = addon.Menu.GetTotalPages(#emotes)
    pageIndex = math.min(math.max(1, pageIndex or 1), totalPages)

    local startIndex = (pageIndex - 1) * addon.Menu.EMOTES_PER_PAGE + 1
    local endIndex = math.min(#emotes, pageIndex * addon.Menu.EMOTES_PER_PAGE)
